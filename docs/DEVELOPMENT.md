...
│
├── writeflow-ai.php # Root plugin entrypoint.
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/justlevine/writeflow-ai.git
   ```
...
- `npm run wp-env run cli -- --env-cwd=wp-content/plugins/writeflow-ai {YOUR_CMD_HERE}`: Run WP-CLI commands in the local environment.
- `npm run wp-env:test run cli -- --env-cwd=wp-content/plugins/writeflow-ai {YOUR_CMD_HERE}`: Run Composer/PHP tooling in the tests container.
