...
namespace WriteFlowAI\Framework;
...
trait AssetLoaderTrait {
	...
	private function register_block_manifest( string $block_path, string $manifest_file ): void {
		...
		if ( ! file_exists( $manifest_path ) ) {
			_doing_it_wrong(
				self::class,
				esc_html__( 'Block manifest file is missing. Blocks will not be registered.', 'writeflow-ai' ),
				'0.0.1'
			);
			return;
		}
	...
	private function get_asset_file( string $filename ): ?array {
		...
		if ( ! file_exists( $asset_file ) ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					esc_html__( 'Asset file for "%s" is missing. The script will not be registered.', 'writeflow-ai' ),
					esc_html( $filename )
				),
				'0.0.1'
			);
		...
		if ( ! is_array( $asset ) ) {
			_doing_it_wrong(
				...
				sprintf(
					/* translators: %s: The asset filename. */
					esc_html__( 'Asset file for "%s" is invalid. The script will not be registered.', 'writeflow-ai' ),
					esc_html( $filename )
				),
				'0.0.1'
			);
