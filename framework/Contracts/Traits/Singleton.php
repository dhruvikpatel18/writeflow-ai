 ...
 *
 * @package WriteFlowAI\Framework\Contracts\Traits
 */

declare( strict_types = 1 );

namespace WriteFlowAI\Framework\Contracts\Traits;
...
trait Singleton {
	...
	final public function __clone() {
		_doing_it_wrong(
			...
			sprintf(
				...
				esc_html__( 'The %s class should not be cloned.', 'writeflow-ai' ),
	...
	final public function __wakeup() {
		_doing_it_wrong(
			...
			sprintf(
				// translators: %s: Class name.
				esc_html__( 'De-serializing instances of %s is not allowed.', 'writeflow-ai' ),
				esc_html( static::class ),
			),
			'0.0.1'
		);
